...
- **SDS文書 → JSON**: PDF/DOCX/XLSX/TXT/**HTML・URL** からテキストを抽出し、厚生労働省のSDS情報交換標準フォーマット v1.0（JSON）に変換します。並列抽出・自動リトライ対応。スキャンPDFは `pdftoppm`+`tesseract` OCRまたはClaude Vision APIフォールバックで処理します。
