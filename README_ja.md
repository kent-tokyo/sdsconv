...
| クレート | 説明 |
|---|---|
| [`sds-converter-core`](./sds_converter_core/) | Rustライブラリ — LLMによる抽出・DOCX/HTML生成・MHLWスキーマ |
| [`sds-converter`](./sds_converter/) | CLIバイナリ — `to-json`・`to-docx`・`to-html`・`to-pdf`・`validate`・`extract-text` サブコマンド |
...
- [ ] crates.io公開（`sds-converter-core` → `sds-converter` の順）

### 計画中
- [ ] GUIアプリケーション（eframe/egui） — ファイル選択・APIキー入力・進捗表示
